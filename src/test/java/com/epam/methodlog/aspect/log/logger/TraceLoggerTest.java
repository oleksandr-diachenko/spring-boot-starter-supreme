...
class TraceLoggerTest {
    ...
    @Test
    void shouldBeApplicableForTrace() {
        boolean applicableFor = logger.isApplicableFor(TRACE);
    ...
    void shouldLogMessage() {
        MessageInfo messageInfo = new MessageInfo(joinPoint, sl4jLogger, TRACE, "Message");

        logger.log(messageInfo);

        verify(sl4jLogger).trace("Message");
    }
}