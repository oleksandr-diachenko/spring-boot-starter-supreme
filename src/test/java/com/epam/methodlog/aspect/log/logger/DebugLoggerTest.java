...
class DebugLoggerTest {
    ...
    @Test
    void shouldBeApplicableForDebug() {
        boolean applicableFor = logger.isApplicableFor(DEBUG);
    ...
    void shouldLogMessage() {
        MessageInfo messageInfo = new MessageInfo(joinPoint, sl4jLogger, DEBUG, "Message");

        logger.log(messageInfo);

        verify(sl4jLogger).debug("Message");
    }
}